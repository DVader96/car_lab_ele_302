...
/*******************************************************************************
* Function Name: Clock_Steering_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_Start(void) 
{
    /* Set the bit to enable the clock. */
    Clock_Steering_CLKEN |= Clock_Steering_CLKEN_MASK;
	Clock_Steering_CLKSTBY |= Clock_Steering_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Steering_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Clock_Steering_CLKEN &= (uint8)(~Clock_Steering_CLKEN_MASK);
	Clock_Steering_CLKSTBY &= (uint8)(~Clock_Steering_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Steering_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_StopBlock(void) 
{
    if ((Clock_Steering_CLKEN & Clock_Steering_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_Steering__CFG3)
        CLK_DIST_AMASK = Clock_Steering_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Steering_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Steering__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Steering_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Steering_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Steering_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_Steering_CLKSTBY &= (uint8)(~Clock_Steering_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_Steering_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_Steering_GetSourceRegister();
    uint16 oldDivider = Clock_Steering_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Steering_CLKEN & Clock_Steering_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Steering_DIV_PTR, clkDivider);
                Clock_Steering_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Steering_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Steering_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Steering_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Steering_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_Steering_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_Steering_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Steering_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_SetModeRegister(uint8 modeBitMask) 
{
    Clock_Steering_MOD_SRC |= modeBitMask & (uint8)Clock_Steering_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Steering_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_Steering_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Steering_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Steering_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Steering_GetModeRegister(void) 
{
    return Clock_Steering_MOD_SRC & (uint8)(Clock_Steering_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Steering_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_Steering_GetDividerRegister();
    uint8 oldSrc = Clock_Steering_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Steering_MOD_SRC |= CYCLK_SSS;
        Clock_Steering_MOD_SRC =
            (Clock_Steering_MOD_SRC & (uint8)(~Clock_Steering_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Steering_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Steering_GetSourceRegister(void) 
{
    return Clock_Steering_MOD_SRC & Clock_Steering_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Steering_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Steering_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_Steering_PHASE = clkPhase & Clock_Steering_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Steering_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Steering_GetPhaseRegister(void) 
{
    return Clock_Steering_PHASE & Clock_Steering_PHASE_MASK;
}
